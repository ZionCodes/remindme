...
<section class="bg-gray-50">
    <div class="py-8 px-4 mx-auto max-w-2xl lg:py-16 bg-white">
        <h2 class="mb-2 text-xl font-semibold leading-none text-gray-900 md:text-2xl dark:text-white">{reminder.name}</h2>
        {#if reminder.description}
        <dl>
            <dt class="mb-2 font-semibold leading-none text-gray-900 dark:text-white">Description</dt>
            <dd class="mb-4 font-light text-gray-500 sm:mb-5 dark:text-gray-400">{reminder.description}</dd>
        </dl>
        {/if}
        ...
        <dl class="flex items-center justify-center space-x-6 mb-4">
            <div>
                <CountdownTimer targetDate={reminder.targetDate} offset={reminder.offset} />
            </div>
        </dl>
...
{:else}
    <p class="p-4 text-center">Reminder not found.</p>
{/if}
